...
- 📚 **多章节文档支持** — 同一 `series` 的文章自动聚合为文档，文章页显示章节目录与上下篇导航，首页列表合并显示为一条文档入口
- 🏷️ **标签系统** — 文章标签展示
...
**多章节文档（系列）：** 一篇长文档拆成多个章节文件时，为每个章节添加 `series`（同一文档共用相同系列名）和 `order`（章节顺序）：
...
order: 3                  # 章节顺序
slug: "rust-guide-03"     # 可选，自定义 URL（否则按日期自动生成）
catalog: "rust-guide"     # 可选，仅在入口章（order 最小的一章）配置：系列目录页路径段
---
```

同一 `series` 的文章会被自动聚合：

- 文章页显示**章节目录**（按 `order` 排序、当前章高亮）与**上一篇/下一篇**导航
- 首页列表只显示一条**文档入口**（带"系列 · 共 N 章"标记），不会列出每个章节
- 点击文档入口进入**目录页**（`/series/<catalog>`，未配置 `catalog` 时使用入口章的 slug），从目录页再进入各章节
